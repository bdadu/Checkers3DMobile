...
function BoardGroup({
  ...
  return (
    ...
    {explosions.map((explosion) => (
      ...
      onComplete={() => setExplosions((prev: Explosion[]) => prev.filter((e: Explosion) => e.id !== explosion.id))}
        />
      ))}
    </group>
  );
...
function GamePage() {
  ...
  const [groupPosition, setGroupPosition] = useState<[number, number, number]>([0.2, -1, BOARD_Z_OFFSET]);
