...
interface PieceData {
  id: string;
  type: 'D' | 'L' | string; // allow generator string fallback
  position: [number, number, number];
  isQueen?: boolean;
}
interface ExplosionData { id: string; position: [number, number, number]; }
...
function GamePage() {
  ...
  setGroupPosition(([x, _y, z]: [number, number, number]) => [x + dx * 0.01, 0, z + dz * 0.01]);
    setDragStart([t.pageX, t.pageY]);
  };
  ...
  return (
    <ImageBackground source={backgroundImage} style={styles.backgroundImage} resizeMode="cover">
      <View
        ...
        {isGameOver && (
          <View style={gameOverOverlayStyles.overlay} pointerEvents="auto">
            <GameOver
              winnerText={scoreDark > scoreLight ? 'WINNER' : 'YOU LOSE'}
