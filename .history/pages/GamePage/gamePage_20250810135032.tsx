...
type GameRouteProp = RouteProp<{ Game: { initialLevel?: Level } }, 'Game'>;
type Route = { params?: { initialLevel?: 'Easy' | 'Medium' | 'Hard' } };
type GamePageProps = { initialLevel?: 'Easy' | 'Medium' | 'Hard' };

function GamePage() {
  const const route = useRoute<GameRouteProp>();
  const initialLevel = params?.initialLevel;
  const [selectedLevel, setSelectedLevel] = useState<'Easy'|'Medium'|'Hard'>(initialLevel ?? 'Easy'); // sau cere mereu param
  ...
  if (!selectedLevel) {
    return (
      <GameLevelSelection
        onSelectLevel={(level) => handleSelectLevel(level as any)}
        onBack={() => setSelectedLevel(null)} // 👈 asta e cheia
      />
    );
  }
